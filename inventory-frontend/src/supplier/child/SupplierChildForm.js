 ...
 */
export class SupplierChildForm extends Component {
    ...
    /**
     * Form action is activated when the form need to save or update supplier child information
     * @param id        supplier Child ID
     * @param parentId  ID of corresponding supplier Group
    ...
    validateSubmit(callback) {
        this.setState({
            ...this.state,
            errors: {
                name: !this.requireField(this.state.data.name) ? "Supplier name is required" : null,
                code: !this.requireField(this.state.data.code) ? "Supplier code is required" : null,
            }
        }, callback)
    }
